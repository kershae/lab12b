...
mydata.df <- mutate(log2.cpm.filtered.norm.df,
                    zero.AVG = (totalRNA_0hr_rep1 + totalRNA_0hr_rep2)/2, 
                    eight.AVG = (totalRNA_8hr_rep1 + totalRNA_8hr_rep2)/2,
                    sixteen.AVG = (totalRNA_16hr_rep1 + totalRNA_16hr_rep2)/2,
                    thirtytwo.AVG = (totalRNA_32hr_rep1 + totalRNA_32hr_rep2)/2,
                    fortyeight.AVG = (totalRNA_48hr_rep1 + totalRNA_48hr_rep2)/2)
                    #now make columns comparing each of the averages above that you're interested in
                    LogFC = (eight.AVG - zero.AVG)) %>% #note that this is the first time you've seen the 'pipe' operator
  mutate_if(is.numeric, round, 2)

datatable(mydata.df[,c(16:20)], 
          extensions = c('KeyTable', "FixedHeader"), 
          filter = 'top',
          options = list(keys = TRUE, 
                         searchHighlight = TRUE, 
